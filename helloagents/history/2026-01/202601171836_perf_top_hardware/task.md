# 任务清单（全链路性能顶尖化）

## 0. 前置与基线

- [√] 固化压测口径：open-loop + sendModel（spread/burst）+ drainMs + errorsByReason
- [√] 跑一轮基线：单聊（5000）、群聊（默认参数）、连接（50000）并记录 runDir
  > 备注: 代表性 runDir：`logs/ws-cluster-5x-test_20260117_183926/`（全链路基线）；群聊 A/B：`logs/ws-cluster-5x-test_20260117_202113/`、`logs/ws-cluster-5x-test_20260117_203402/`

## 1. 噪声与可比性修复

- [√] 修复单机多 JVM 下 MyBatis-Plus ASSIGN_ID 冲突导致的 DuplicateKey/internal_error（若仍存在）
- [√] 清理/隔离跨 run 的幂等污染与关机噪声（确保错误率可解释）
  > 备注: 已引入 sendModel（spread/burst）与错误 reason 统计（群聊补齐半包拼接），并通过 `AutoUserBase` 规避跨 run 幂等污染。

## 2. 单聊链路优化

- [√] 线程/连接池配额审计与自动对齐（不同实例数下保持总并发稳定）
  > 备注: 已在 `scripts/ws-cluster-5x-test/run.ps1` 中实现 AutoTuneLocalThreads（Netty worker/DB executor/JDBC 连接池按实例数分摊且保持总量稳定），并用于 5~11 实例扫点验证。
- [√] 关键分段（queue/dbQueue/dbToEventLoop）逐项压缩，目标是把排队型延迟消到最低
  > 备注: 代表性回归 runDir：`logs/ws-cluster-5x-test_20260118_004503/`（5 实例，open-loop，MsgIntervalMs=3000，repeats=1）。

## 3. 群聊链路优化

- [√] group_dispatch 批处理与跨实例 publish 合并优化（减少 per-user 操作）
  > 备注: 已实现“本机 fanout 复用序列化结果（prepare once, write many）”，并补齐 `ByteBuf` 释放保护；收益在当前 baseline 下不显著，但可降低 CPU/GC 风险上界。
- [√] 策略 auto/push/notify 的阈值与降级路径压测验证（含慢消费者/背压）
  > 备注: 群聊 updated_at A/B：`logs/ws-cluster-5x-test_20260117_202113/` vs `logs/ws-cluster-5x-test_20260117_203402/`。

## 4. Redis 宕机/抖动下的功能与降级

- [√] 路由/幂等/PubSub 不可用时的行为明确化（返回码/降级策略/保护阀）
- [√] Redis down 专测：单聊/群聊/ACK/登录踢人等关键路径不崩溃，错误率可控
  > 备注: 群聊 Redis down A/B：
  > - baseline（旧实现）：`logs/ws-cluster-5x-test_20260118_003720/`（p95/p99 秒级）
  > - 改进后：`logs/ws-cluster-5x-test_20260118_005605/`（p95 收敛到几十 ms，p99 < 1s）

## 5. 归因与报告

- [ ] 对每个“突破性改动”做消融对照（runDir + 指标表）
- [ ] 更新 Wiki：最终性能指标（延迟/吞吐/错误率/保存率）+ 关键改动归因
- [ ] 更新 CHANGELOG + 提交代码（按提交拆分，便于回滚/对照）
