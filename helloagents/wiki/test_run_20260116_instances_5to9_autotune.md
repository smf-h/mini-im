...
- 这轮“巨大突破”的本质：**限制每实例 Netty worker/eventLoop 线程数 + DB/JDBC 总并发稳定化**，避免“实例数上去→线程数爆炸→上下文切换/GC/锁竞争→DB 排队→3s 超时→internal_error”。
 - ⚠️ 口径补充：本文件里 `p50≈9ms` 的“观感巨大突破”也强依赖压测端 `sendModel=spread`（把每 3 秒齐发的微突发摊平）；见 `helloagents/wiki/test_run_20260116_ablation_sendmodel_vs_autotune.md:1` 的 2×2 消融对照。
