...
- 可靠性：✅ `ACK(saved)+ACK(delivered/read)` 链路正常；⚠️ 离线/兜底补发用例依赖配置 `im.cron.resend.enabled=true` 才会通过
...
解析口径：当时网关有一套“分段打点日志”（`ws_perf ...`）用于拆分排队/DB/回切等耗时；该能力现已移除，本文仅保留当时的对照结论与数据引用：
...
- 入站队列门禁（现已移除）：当时用于限制单连接的串行队列排队长度，代价是更多 `ERROR reason=server_busy`
...
- 说明：文中提到的 WS encode offload / 分段打点相关开关与配置现已移除，保留本段仅用于解释当时的尝试背景
...
目的：当时评估“将 JSON 编码从 eventLoop 迁出”的收益（相关开关/脚本参数后续已移除，本段仅保留对照结论）。
...
结论：在当时实现与当时机器环境下，该尝试对单聊尾延迟收益为负；如未来再次引入类似优化，建议先补齐队列排队/CPU 争用的观测与参数校准再做对照。
